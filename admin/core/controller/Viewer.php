	...
	public static function addView($servid,$field,$table){
		if(Session::getUID()!=""){
			...
			if(self::checkView($servid,$field,$table)==true){
				$con = Database::getCon();
					 $sql = "insert into $table (ADMIN_ID, $field,REALIP,CREADO_EN) value (".Session::getUID().",$servid,\"".IpLogger::getRealIP()."\",NOW())";
				if($con->query($sql)){
					return true;
				}else{
					return false;
			...
			if(self::checkView($servid,$field,$table)==true){
				$con = Database::getCon();
				$tim = time();
				$sql = "insert into $table ($field,REALIP,CREADO_EN) value ($servid,\"".IpLogger::getRealIP()."\",NOW())";
				if($con->query($sql)){
					return true;
				}else{
					return false;
...
public static function countAllFromDay($day){
	$con = Database::getCon();
	$sql = "select count(*) as c  from VER_PRODUCTO where date(CREADO_EN)=\"$day\"";
	$c = 0;
	$query = $con->query($sql);
	while($r=$query->fetch_array()){				
		$c = $r['c'];
...
public static function countByAdIdFromToday($ad_id){
	$con = Database::getCon();
	$sql = "select count(*) as c  from post_view where post_id=".$ad_id." and date(CREADO_EN)=date(NOW())";
	$c = 0;
	$query = $con->query($sql);
	while($r=$query->fetch_array()){				
		$c = $r['c'];
...
//		$sql = "select serviceview.id as svid, iplog.id as ilid serviceview inner join iplog on (serviceview.iplog_id=iplog.id) where $field=$servid and order by created_at desc limit 1";
		$sql = "select * from $table where REALIP='".IpLogger::getRealIP()."' and $field='$servid' order by CREADO_EN desc limit 1";
		$query = $con->query($sql);
		$found=false;
		$ca = 0;
		while($r=$query->fetch_array()){
			$found=true;
			$ca = strtotime($r['CREADO_EN']);
		}
if($found==true){
		$ca2 = $ca + (24)*3600;
		if(time()>=$ca2){
